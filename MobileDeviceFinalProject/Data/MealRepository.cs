...
namespace MobileDeviceFinalProject.Data
{
    public class MealRepository
    {
        ...
        public async Task<List<MealEntry>> GetByDateAsync(DateTime date)
        {
            ...
            await connection.OpenAsync();
            var cmd = connection.CreateCommand();
            cmd.CommandText = "SELECT * FROM MealEntry WHERE date(LoggedAt) = date(@date) ORDER BY LoggedAt DESC";
        ...
        public async Task SaveAsync(MealEntry entry)
        {
            ...
            var cmd = connection.CreateCommand();
            cmd.CommandText = @"
                INSERT INTO MealEntry (FoodName, Calories, ProteinG, CarbsG, FatG, ServingQty, ServingUnit, LoggedAt, PhotoPath)
                VALUES (@FoodName, @Calories, @ProteinG, @CarbsG, @FatG, @ServingQty, @ServingUnit, @LoggedAt, @PhotoPath)";
